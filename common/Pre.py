...
def cFromCsvToJson(csv_path,profileList):
...
def dEnvReplaceYaml(yaml_file, new_yaml_file,profileList):
...
def pre(module):
    ...
    for i in files:
        if i.endswith('.csv'):
            ...
            profileList = []  # 从csv中读取用例参数, 以字典的列表形式 存放到 profileList , 一个字典就是一个用例
        ...
        cFromCsvToJson(csv_path=f'csvdata/{module}/{i}',profileList=profileList)
        dEnvReplaceYaml(yaml_file=f'testcases/{module}/format/{ff}',
                        new_yaml_file=f'hotdata/{module}/{i[:-4]}_nyf.yaml',profileList=profileList)
